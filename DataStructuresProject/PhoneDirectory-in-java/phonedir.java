...
case "a": showAllRecords();
break;
case "d": deleteCurrentRecord();
break;
case "f": changeFirstName();
break;
...
case "n": addNewRecord();
break;
case "p": changePhoneNumber();
break;
case "s": selectRecord(); break;
default:
System.out.println("The option you've selected is not available. Please choose from the " + "available options.");
break;
}
...
for (int i =
individual.getLastName().length(); i < 20; i++) {
System.out.print(" ");
}
...
for (int i =
individual.getPhoneNumber().length(); i < 20; i++)
{
System.out.print(" ");
...
if (persons.size() == 0 || this.currentRecord ==
-1) {
System.out.println("No current record");
return;
}
...
String s1 = newFirstName.substring(0,
1).toUpperCase();

newFirstName = s1 +
newFirstName.substring(1);
...
String s1 = newLastName.substring(0,
1).toUpperCase();
newLastName = s1 +
newLastName.substring(1);
Person current = persons.get(currentRecord);
persons.remove(currentRecord);
...
numExists =
checkIfPhoneNumberExists(phoneNum);
if (numExists) {
System.out.println("Number already exists, try another.");
System.out.println("You can enter 'q' to quit, 'a' to display all records or add a new number to " + "try again.");
}
}
Person current = this.persons.get(
this.currentRecord );
...
// i.e. 123456789 OR 123-456-7890 OR 123.456.7890 OR 123 456 7890 OR (123) 4567890
...
Person individual = new Person(first, last,
number);
getIndexOfInsertion(individual);

this.persons.add(this.currentRecord,
individual);
displayCurrentRecord();
}
...
int indexOfPerson = getIndexOfPerson( new
Person(first, last, phoneNumber) );
if (indexOfPerson == -1) {
...
String currentPerson = person.getFirstName()
+ " " + person.getLastName() + " " +
person.getPhoneNumber();
// Invariant: 0 <= i < persons.size()
for (int i = 0; i < persons.size(); i++) {
String compareToPerson =
persons.get(i).getFirstName() + " " +
persons.get(i).getLastName() + " " +
persons.get(i).getPhoneNumber();
if
(currentPerson.toLowerCase().compareTo(
compareToPerson.toLowerCase() ) == 0) {
return i;
}
}
return -1;
}
private void getIndexOfInsertion(Person
current) {
String currentPerson = current.getLastName()
...
/** An individual's phone number in the
format xxx-xxx-xxxx. */
